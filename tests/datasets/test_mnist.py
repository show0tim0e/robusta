...
def _ensure_dataset_artifact() -> Path:
    ...
    if DATASET_PATH.exists():
        try:
            ...
            regenerate = not ("x" in data and "y" in data and isinstance(data["x"], torch.Tensor) and isinstance(data["y"], torch.Tensor) and data["x"].shape == (10000, 1, 28, 28) and data["y"].shape == (10000,))
        except Exception:
            regenerate = True

    if regenerate:
        generated_path = Path(download_mnist())
